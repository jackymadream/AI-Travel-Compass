#!/usr/bin/env python3
"""
Phase 6.1 — Seed 25–30 popular countries with photos, tags, and top cities.

Prerequisites
-------------
1. Apply ``scripts/migrate_phase6_countries.sql`` in the Supabase SQL Editor
   (adds slug, photo_url, tags, top_cities).
2. Set SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY in ``.env``.
3. Run::

     python scripts/seed_countries.py

Upserts countries on ``iso_code`` and cities on ``(country_id, slug)``.
After seeding new cities, optionally run ``python scripts/embed_destinations.py``
so hybrid search covers the expanded set.
"""

from __future__ import annotations

import json
import os
import sys
from pathlib import Path
from typing import Any

from dotenv import load_dotenv
from supabase import Client, create_client

ROOT_DIR = Path(__file__).resolve().parent.parent
SEED_FILE = ROOT_DIR / "data" / "countries_phase6.json"

REQUIRED_LOCALES = ("en", "zh-HK", "ja")
VALID_SEASONS = {"spring", "summer", "autumn", "winter"}

COUNTRY_COLUMNS = {
    "iso_code",
    "slug",
    "name",
    "description",
    "safety_index",
    "avg_daily_cost_usd",
    "best_travel_season",
    "region_tags",
    "tags",
    "photo_url",
    "top_cities",
}

CITY_COLUMNS = {
    "slug",
    "name",
    "description",
    "safety_index",
    "avg_daily_cost_usd",
    "best_travel_season",
    "latitude",
    "longitude",
    "tags",
}


def load_config() -> tuple[str, str]:
    load_dotenv(ROOT_DIR / ".env")
    url = os.getenv("SUPABASE_URL", "").strip()
    key = os.getenv("SUPABASE_SERVICE_ROLE_KEY", "").strip()
    missing = [
        name
        for name, value in [("SUPABASE_URL", url), ("SUPABASE_SERVICE_ROLE_KEY", key)]
        if not value
    ]
    if missing:
        print(
            "Missing required environment variables: "
            + ", ".join(missing)
            + "\nApply migrate_phase6_countries.sql, then set .env from Supabase API settings.",
            file=sys.stderr,
        )
        sys.exit(1)
    return url, key


def assert_i18n(value: Any, path: str) -> None:
    if not isinstance(value, dict):
        raise ValueError(f"{path} must be an object with locales {REQUIRED_LOCALES}")
    for locale in REQUIRED_LOCALES:
        text = value.get(locale)
        if not isinstance(text, str) or not text.strip():
            raise ValueError(f"{path} missing non-empty '{locale}'")


def assert_best_travel_season(value: Any, path: str) -> None:
    if not isinstance(value, dict):
        raise ValueError(f"{path} must be an object")
    seasons = value.get("seasons")
    months = value.get("months")
    if not isinstance(seasons, list) or not seasons:
        raise ValueError(f"{path}.seasons must be a non-empty array")
    if any(season not in VALID_SEASONS for season in seasons):
        raise ValueError(f"{path}.seasons contains invalid values")
    if not isinstance(months, list) or not months:
        raise ValueError(f"{path}.months must be a non-empty array")
    if any(not isinstance(m, int) or m < 1 or m > 12 for m in months):
        raise ValueError(f"{path}.months must be integers 1–12")
    assert_i18n(value.get("label"), f"{path}.label")


def validate_seed(data: dict[str, Any]) -> None:
    countries = data.get("countries")
    if not isinstance(countries, list) or not (25 <= len(countries) <= 40):
        raise ValueError("countries_phase6.json must contain 25–40 countries")

    seen_iso: set[str] = set()
    seen_slug: set[str] = set()
    for i, country in enumerate(countries):
        prefix = f"countries[{i}]"
        for field in (
            "iso_code",
            "slug",
            "name",
            "description",
            "best_travel_season",
            "photo_url",
            "tags",
            "top_cities",
        ):
            if field not in country:
                raise ValueError(f"{prefix} missing '{field}'")
        iso = country["iso_code"]
        if len(iso) != 2:
            raise ValueError(f"{prefix}.iso_code must be a 2-letter code")
        if iso in seen_iso:
            raise ValueError(f"Duplicate iso_code: {iso}")
        seen_iso.add(iso)
        slug = country["slug"]
        if not isinstance(slug, str) or not slug.strip():
            raise ValueError(f"{prefix}.slug must be non-empty")
        if slug in seen_slug:
            raise ValueError(f"Duplicate country slug: {slug}")
        seen_slug.add(slug)
        assert_i18n(country["name"], f"{prefix}.name")
        assert_i18n(country["description"], f"{prefix}.description")
        assert_best_travel_season(
            country["best_travel_season"], f"{prefix}.best_travel_season"
        )
        if not (1 <= int(country["safety_index"]) <= 5):
            raise ValueError(f"{prefix}.safety_index must be 1–5")
        if float(country["avg_daily_cost_usd"]) <= 0:
            raise ValueError(f"{prefix}.avg_daily_cost_usd must be > 0")
        if not isinstance(country["tags"], list) or not country["tags"]:
            raise ValueError(f"{prefix}.tags must be a non-empty array")
        photo = country["photo_url"]
        if not isinstance(photo, str) or "unsplash.com" not in photo:
            raise ValueError(f"{prefix}.photo_url must be an Unsplash URL")
        top = country["top_cities"]
        if not isinstance(top, list) or not (2 <= len(top) <= 4):
            raise ValueError(f"{prefix}.top_cities must have 2–4 entries")
        cities = country.get("cities", [])
        if not isinstance(cities, list) or not (2 <= len(cities) <= 4):
            raise ValueError(f"{prefix}.cities must have 2–4 entries")
        for j, city in enumerate(cities):
            cprefix = f"{prefix}.cities[{j}]"
            for field in ("slug", "name", "description", "best_travel_season"):
                if field not in city:
                    raise ValueError(f"{cprefix} missing '{field}'")
            assert_i18n(city["name"], f"{cprefix}.name")
            assert_i18n(city["description"], f"{cprefix}.description")
            assert_best_travel_season(
                city["best_travel_season"], f"{cprefix}.best_travel_season"
            )


def pick_fields(record: dict[str, Any], allowed: set[str]) -> dict[str, Any]:
    return {key: record[key] for key in allowed if key in record}


def upsert_country(client: Client, country: dict[str, Any]) -> str:
    payload = pick_fields(country, COUNTRY_COLUMNS)
    payload.setdefault("region_tags", [])
    payload.setdefault("tags", [])
    payload.setdefault("top_cities", [])
    response = (
        client.table("countries")
        .upsert(payload, on_conflict="iso_code")
        .execute()
    )
    rows = response.data or []
    if not rows:
        lookup = (
            client.table("countries")
            .select("id")
            .eq("iso_code", payload["iso_code"])
            .single()
            .execute()
        )
        if not lookup.data:
            raise RuntimeError(
                f"Country upsert returned no row for iso_code={payload['iso_code']}"
            )
        rows = [lookup.data]
    return rows[0]["id"]


def upsert_city(client: Client, city: dict[str, Any], country_id: str) -> None:
    payload = pick_fields(city, CITY_COLUMNS)
    payload["country_id"] = country_id
    payload.setdefault("tags", [])
    client.table("cities").upsert(
        payload,
        on_conflict="country_id,slug",
    ).execute()


def seed(client: Client, data: dict[str, Any]) -> tuple[int, int]:
    country_count = 0
    city_count = 0
    for country in data["countries"]:
        country_id = upsert_country(client, country)
        country_count += 1
        print(f"  {country['slug']} ({country['iso_code']}) → {country_id}")
        for city in country.get("cities", []):
            upsert_city(client, city, country_id)
            city_count += 1
            print(f"    · {city['slug']}")
    return country_count, city_count


def main() -> None:
    print("GenAI Travel Compass — Phase 6.1 country seeder")
    print(f"Seed file: {SEED_FILE.relative_to(ROOT_DIR)}")
    if not SEED_FILE.exists():
        print(f"Missing {SEED_FILE}", file=sys.stderr)
        sys.exit(1)

    url, key = load_config()
    client = create_client(url, key)
    with SEED_FILE.open(encoding="utf-8") as f:
        data = json.load(f)

    try:
        validate_seed(data)
    except ValueError as exc:
        print(f"Invalid seed data: {exc}", file=sys.stderr)
        sys.exit(1)

    print(f"\nSeeding {len(data['countries'])} countries…")
    try:
        country_count, city_count = seed(client, data)
    except Exception as exc:  # noqa: BLE001
        print(
            f"Seed failed: {exc}\n"
            "Did you apply scripts/migrate_phase6_countries.sql?",
            file=sys.stderr,
        )
        sys.exit(1)

    print(f"\nDone. Upserted {country_count} countries and {city_count} cities.")


if __name__ == "__main__":
    main()
